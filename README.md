...
- **TOC 目录**:输入 `[toc]` 自动生成可点击目录,跟随标题实时刷新,点击滚动定位
...
### 文件管理
- 原生对话框打开 / 保存 / 另存为(`⌘S` / `⌘⇧S`)
- **拖拽打开**:拖 `.md` 进窗口直接打开;拖文件夹设为工作区;拖图片插入光标处
- **系统文件关联**:双击 `.md` 文件直接用 Yymd 打开;已运行时双击第二个文件在同一窗口打开(macOS 自动尝试设为默认打开方式)
- **会话恢复**:重启自动打开上次的文件
- **快速打开**(`⌘P`):最近文件 + 全目录模糊搜索
...
下一版本需求池(图片缩放、多标签页、Pandoc 导出…)见 [ROADMAP.md](./ROADMAP.md)。
